...
- `agent/macos_window_actions.py` owns macOS window action dispatch. It may read the frontmost window full-screen flag, but movement itself is delegated to configured local window-management shortcuts.
...
- macOS system window actions are built-in System Actions backed by configured local shortcuts. The current small set is `窗口左半屏`, `窗口右半屏`, `窗口左移`, `窗口右移`, `窗口最大化`, and `窗口居中`. They no longer calculate or write window coordinates. If the frontmost window is full screen, the engine sends the configured full-screen toggle first, waits for the Space transition, and then sends the requested window-management shortcut.
